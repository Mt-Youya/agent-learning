...
| Modern accessible React foundation        | `@base-ui/themes`                                          | Primitives + polished theme                                   |
...
- **Allowed libraries (priority order):** `@phosphor-icons/react`, `hugeicons-react`, `@base-ui/react-icons`, `@tabler/icons-react`.
...
- When using a design system with built-in theming (Base Themes, shadcn/ui with `<Theme>`), set the theme ONCE in `layout.tsx` or the page root. Do not let individual sections override.
...
- **CSS variables** (for shadcn/ui, Base Themes, or component libraries with theming): define semantic tokens (`--surface`, `--surface-elevated`, `--text-primary`, `--accent`) and swap values under `[data-theme="dark"]` or `@media (prefers-color-scheme: dark)`.
...
- **NO hand-rolled SVG icons.** Use Phosphor / HugeIcons / Base / Tabler. Lucide on explicit request only.
...
- [ ] **Icons** from an allowed library only (Phosphor / HugeIcons / Base / Tabler), no hand-rolled SVG paths?
...
- https://www.base-ui.com/themes/docs/components/theme
- https://www.base-ui.com/themes/docs/components/card
- https://github.com/base-ui/themes
